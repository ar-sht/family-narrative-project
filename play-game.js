      ...
      dinnerChoice.addEventListener('click', function () {
        ...
        apartmentButton.addEventListener('click', function () {
          ...
          trainChoice.addEventListener('click', function () {
            ...
            document.getElementById(`${currentStoryId.length - 1}`).style.color = 'grey'
            storyField.innerHTML += nextStory
            const americaChoice = document.getElementById('america')
            americaChoice.addEventListener('click', function () {
              currentStoryId.push(Number(americaChoice.value))
              nextStory = get_story(currentStoryId, nataStories)
              document.getElementById(`${currentStoryId.length - 1}`).style.color = 'grey'
              storyField.innerHTML += nextStory
            })
            const italyChoice = document.getElementById('italy')
            italyChoice.addEventListener('click', function () {
              currentStoryId.push(Number(italyChoice.value))
              nextStory = get_story(currentStoryId, nataStories)
              document.getElementById(`${currentStoryId.length - 1}`).style.color = 'grey'
              storyField.innerHTML += nextStory
            })
